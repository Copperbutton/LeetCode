...
public class StringToInteger {
    public int atoi(String str) {
        ...
        int base = 0, sign = 1, indx = 0, len = chs.length;
        while (indx < len && chs[indx] == ' ')
            indx++;
        if (indx < len && (chs[indx] == '+' || chs[indx] == '-')) {
            sign = (chs[indx++] == '-') ? -1 : 1;
        }
        while (indx < len && (chs[indx] >= '0' && chs[indx] <= '9')) {
            if (base > Integer.MAX_VALUE / 10
                    || (base == Integer.MAX_VALUE / 10 && chs[indx] > '7'))
                return (sign == -1) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            base = base * 10 + (chs[indx++] - '0');
        }
        return base * sign;
    }
}